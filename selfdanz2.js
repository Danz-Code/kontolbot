...
const DeveloperNumber = ["6281319326901@s.whatsapp.net","62895325677273@s.whatsapp.net","62895326098599@s.whatsapp.net"]
/************************************/
...
client.on('group-participants-update', async(chat) => {
      if (!welkom2.includes(chat.jid)) return
        try {
            ...
            if (chat.action == 'add') {
                ...
                ini_img = await getBuffer(`http://api.lolhuman.xyz/api/welcomeimage?apikey=${LolhumanKey}&img=${pp_user}&text=${ini_user}`)
                group_info = await client.groupMetadata(chat.jid)
                welkam = `*Halo ngab/ngen ( ngen artinya neng yah Kak :) )* ${mem.split('@')[0]}, Welcome to ||${group_info.subject}|| Jangan lupa intro, Bantu follow ig owner\n@ff.firebold\n@danz.nih._\n*Terimakasih yg udah follow. tinggal donasi yah Kak :)*`
	...
	try {
		ppimg = await client.getProfilePicture(`${num.split('@')[0]}@c.us`)                                                                                                      
                ...
                ini_out = `*Alhamdulillah Beban grup keluar🗿*${ini_user.split('@')[0]}`
...
/*			const isOwner = ownerNumber.includes(sender)*/
			const isDeveloper = !mek.key.fromMe
    ...
    premiumcode.splice(prem,1*/
			...
			switch(command) {
                ...
                if (isBadWord) return reply('*Fitur BadWord sudah aktif sebelum nya*')
                   ...
                   case 'publicc':
               if (!mek.key.fromMe) return  reply('Owner kah?')
...
case 'public':
          	...
          	anu  = {contextInfo:{'stanzaId': "B826873620DD5947E683E3ABE663F263", 'participant': `$num.split('@')s.whatsapp.net`, 'remoteJid': '6283136505591-1614953337@g.us', 'quotedMessage': {"imageMessage": {"caption": `「 𝐑𝐔𝐍𝐓𝐈𝐌𝐄 」\n*${kyun(uptime)}*`, 'jpegThumbnail': fs.readFileSync('./image/odc.jpeg')}}}}
...
case 'sewa':
                    ...
                    if (args[0] === 'add') 
                       addSewaGroup(chat.id , args[1], _sewa)
                         ...
                         reply(` *「 SEWA ADDED 」*\n\n➸ *ID*: Null\n➸ *Expired*: ${(toMs(args[1])).days} day(s) ${(toMs(args[1])).hours} hour(s) ${(toMs(args[1])).minutes} minute(s)\n\nBot Akan Keluar Secara Otomatis\nDalam waktu yang sudah di tentukan`)
...
case 'giftlimit': 
				...
				const nomerr = args[0].replace('@','')
                		const jmla = args[1]
                		if (jmla <= 1) return reply(`minimal gift limit adalah 1`)
				if (jmla >= 25) return reply(`maximal gift limit adalah 25`)
                		if (isNaN(jmla)) return reply(`limit harus berupa angka`)
                		if (!nomerr) return reply(`maaf format salah\nmasukan parameter yang benar\ncontoh : ${prefix}giftlimit @62895325677273 20`)
                		const cysz = nomerr + '@s.whatsapp.net'
                		var found = false
...
case 'play2':
		   if (!isRegistered) return reply(ind.noregis())
		    ...
		    ini_txt = `Title : ${get_info.title}\n`
                    ...
                    client.sendMessage(from, get_audio, audio, { mimetype: 'audio/mp4', filename: `${get_info.title}.mp3`, quoted: lol })
...
case 'ytmp3':
		...
		case 'lirik': 
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					...
					teks = body.slice(6)
					buffer = await getBuffer(`http://api.lolhuman.xyz/api/lirik?apikey=${LolhumanKey}&query=despacito=${teks}`, {method: 'get'})
		...
		case 'alay':
				...
				if (isBanned) return reply('Yah kamu sudah dibanned. Gk keren deh :(')
					if (args.length < 1) return reply('kasih teks lah^_^!!!')
					data = await fetchJson(`http://lolhuman.herokuapp.com/api/alay?apikey=beta&text=${Lolhuman}`)
					reply(data.result)
...
if (!isRegistered) return reply('Regis Dlu')
			await reply ('Wait lagi di proses')
                    if (args.length == 0) return reply(`Example: ${prefix + command} vt.tiktok.com/ZSwWCk5o/`)
                    ini_link = args[0]
                    get_audio = await getBuffer(`http://api.lolhuman.xyz/api/tiktokmusic?apikey=${LolhumanKey}&url=https://${ini_link}`)
...
case 'welcome2':
        ...
        case 'antilinkgroup':
					...
					if (Number(args[0]) === 1) {
						...
						client.sendMessage(from,`Perhatian kepada seluruh member group. anti snack video sudah diaktifkan, apabila mengirim link group maka akan otomatis kick`, text)
					} else if (Number(args[0]) === 0) {
...
case 'antilinkown':
				...
				antilink.push(from)
				fs.writeFileSync('./database/kelompok/antilink.json', JSON.stringify(antilink))
				await reply('Sukses mengaktifkan anti link group di group ini ✔️')
				client.sendMessage(from,`Perhatian kepada seluruh member anti link group aktif apabila anda mengirim link group anda akan di kick dari group`, text)
				} else if (Number(args[0]) === 0) {
				if (!isAntilink) return reply('Mode anti link group sudah disable')
				antilink.splice(from, 1)
