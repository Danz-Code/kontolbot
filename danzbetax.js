...
client.on('credentials-updated', () => {
   ...
   fs.writeFileSync('./sessionx.json', JSON.stringify(authInfo, null, '\t'))
})
fs.existsSync('./sessionx.json') && client.loadAuthInfo('./sessionx.json')
client.connect();



/*client.on('group-participants-update', async (chat) => {
	if (!welkom3.includes(chat.jid)) return
	try {
		const mdata = await client.groupMetadata(chat.jid)
		console.log(chat)
		if (chat.action == 'add') {
...
pushnem = client.contacts[num] != undefined ? client.contacts[num].notify = undefined ? PhoneNumber('+' + num.replace('@s.whatsapp.net', '')).getNumber('international') : client.contacts[num].notify || client.contacts[num].vname : PhoneNumber('+' + num.replace('@s.whatsapp.net', '')).getNumber('international')
			try {
				ppimg = await client.getProfilePicture(`${chat.participants[0].split('@')[0]}@c.us`)
			} catch {
				ppimg = 'https://i.ibb.co/VQgzwW7/20210407-005215.jpg'
			}
				exec(`magick './src/wel.jpg' -gravity west -fill '#ff2fa2' -font './src/font-gue.ttf' -size 1280x710 -pointsize 75 -interline-spacing 7.5 -annotate +460-45 '${pushnem}' -pointsize 35 -annotate +460+83 '${jm} ${calender}' -pointsize 50 -annotate +460+200 'Welcome To ${mdata.subject}' '${ppimg}' -resize %[fx:t?u.w*0.2:u.w]x%[fx:?u.h*0.2:u.h] -gravity center -geometry -430+70 -composite 'wel.jpg'`)
...
Ketik ${prefix}listmenu Untuk melihat menu.`
			...
			client.sendMessage(mdata.id, fs.readFileSync('wel.jpg'), MessageType.image, { caption: welkam, contextInfo: { mentionedJid: [num] }})
			})
...
pushnem = client.contacts[num] != undefined ? client.contacts[num].notify = undefined ? PhoneNumber('+' + num.replace('@s.whatsapp.net', '')).getNumber('international') : client.contacts[num].notify || client.contacts[num].vname : PhoneNumber('+' + num.replace('@s.whatsapp.net', '')).getNumber('international')
			try {
				...
				exec(`magick './src/lev.jpg' -gravity west -fill '#ff2fa2' -font './src/font-gue.ttf' -size 1280x710 -pointsize 70 -interline-spacing 7.5 -annotate +460-45 '${pushnem}' -pointsize 35 -annotate +460+83 '${jamny} ${calender}' -pointsize 50 -annotate +460+200 'Leaving from ${mdata.subject}' '${ppimg}' -resize %[fx:t?u.w*0.2:u.w]x%[fx:?u.h*0.2:u.h] -gravity center -geometry -430+70 -composite 'wel.jpg'`)
				.on('error', () => reply('error'))
				.on('exit', () => {
			client.sendMessage(mdata.id, fs.readFileSync('wel.jpg'), MessageType.image, {caption: `*Dadah 
Beban group 👋*@${num.split('@')[0]}\n`, contextInfo: { mentionedJid: [num] }})
			})
		}
	} catch (e) {
		console.log(e)
...
const pushname = anu ? client.user.name : contx.notify || contx.vname || contx.name || '-'
...
const mdatal = await client.groupMetadata(anu.jid)
            if (anu.action == 'add') {
                ...
                anu_img = await getBuffer(`https://bx-hunter.herokuapp.com/api/welcome?name=${pushname}&mem=${groupMemberx.length}&icon=http://tools.bandungxploiter.site/uploads/60b0a49c733de.jpg&gcname=${groupNamex}&picurl=${pp_user}&apikey=ikygans`)
...
Ketik ${prefix}menu Untuk melihat menu.`
            ...
            if (anu.action == 'remove') {
            	num = anu.participants[0]
                anu_user = client.contacts[mem]
                anu_img = await getBuffer(`https://bx-hunter.herokuapp.com/api/goodbye?name=${pushname}&mem=${groupMemberx.length}&icon=http://tools.bandungxploiter.site/uploads/60b0a49c733de.jpg&gcname=${groupNamex}&picurl=${pp_user}&apikey=ikygans`)
                group_info = await client.groupMetadata(anu.jid)
...
if (budy.includes("chat.whatsapp.com/")) {
		...
		if (isOwner) return reply('Apa?')
                if (isGroupAdmins) return reply('karena kamu adalah admin group, bot tidak akan kick kamu')
                client.updatePresence(from, Presence.composing)                                                                         
                reply(`Link Group Terdeteksi ${pushname} anda saya peringatkan untuk tidak spam link.`)
}
...
if (isDeveloper){
		...
		if (!public){
			...
			if (!isGroup && isCmd) console.log('\x1b[1;31m~\x1b[1;37m>', '[\x1b[1;32mEXECUTE\x1b[1;37m]', 'Script By : Danz', time, color(command), 'from', color(sender.split('@')[0]), 'args :', color(args.length)), 'PRIVATE MSG : DANZ'
			if (!isGroup && !isCmd) console.log('\x1b[1;31m~\x1b[1;37m>', '[\x1b[1;31mGAGAL\x1b[1;37m]','Script By : Danz', time, color('Message'), 'from', color(sender.split('@')[0]), 'args :', color(args.length)),'PRIVATE MSG : DANZ'
	...
	case 'ktpfake':
		...
		case 'attp2':
		   if (!isRegistered) return reply(ind.noregis())
                    ...
                    ini_txt = args.join(" ")
                    ini_buffer = await getBuffer(`https://hardianto-chan.herokuapp.com/api/maker/2ttp?query=${ini_txt}&apikey=hardianto`)
...
case 'art':
		...
		case 'play':   
				...
				reply(ind.wait())
				play = body.slice(6)
				buffer = await fetchJson(`https://xinzbot-api.herokuapp.com/api/ytplaymp3/?q=dj_desa&apikey=XinzBot`)
				if (buffer.error) return reply(buffer.error)
...
/*				case 'tiktoknowm': 
				...
				if (!isUrl(args[0]) && !args[0].includes('tiktok.com')) return reply(mess.error.lv)
					 ige = body.slice(12)
                     buffer = await getBuffer(`https://api.lolhuman.xyz/api/tiktok?apikey=${LolhumanKey}&url=${args[0]}`, {method: 'get'})
					if (buffer.error) return reply(buffer.error)
...
◪ Version Bot : 0.2.1 ( Beta ) 
 Note : ${notebest}
  ...
  ❏ Xp/Progres🔗 : ${getLevelingXp(sender)}/${reqXp} 
  ❏ Status🔑 : *${prema}*
  Your Number With Link : wa.me/${sender.split("@")[0]}

Note : Don't spam command or you Get Banned!! 
Jangan spam command atau kamu akan di banned
...
╠ Ke Owner : Ketik #lpr [ Jenis bug ] < 3 >
╰════─⊱⸨ *DANZ BOT* ⸩⊰─════════──
...
Limit lapor adalah : 1 ( DIRESET SETIAP JAM 00:00 ) 

Masih gk paham Juga? Gk usah pake bot

Auto Banned Status : { ${banstatus} } 
( JANGAN SPAM / AUTO BANNED ON!! ) 

╭─━━━━❰ *SIMPLE FITURE* ❱━━━━─╮
│*${prefix}lpr* [ JENIS BUG ] <1(LIMIT LAPORAN) >
...
⎆${prefix}afk [ Alasan ]
⎆ ${prefix}linkgc
⎆ ${prefix}add [62] ( Admin Group ) 
⎆ ${prefix}kick [@tag] ( Admin Group ) 
⎆ ${prefix}setname ( Admin Group ) 
⎆ ${prefix}setdesc ( Admin Group ) 
⎆ ${prefix}setpp ( Admin Group ) 
⎆ ${prefix}leave ( Admin Group ) 
⎆ ${prefix}mining
©  ${prefix}event [ 0/1 ] 
⎆ ${prefix}level
⎆ ${prefix}grup [buka/tutup)
⎆ ${prefix}welcome [1/0] ( Admin Group ) 
💲 ${prefix}welcome2 [1/0]
⎆ ${prefix}nsfw [1/0] ( Admin Group ) 
⎆ ${prefix}antilink [1/0] ( Admin Group ) 
⎆ ${prefix}antilinkgroup [1/0] ( Admin Group ) 
⎆ ${prefix}leveling [1/0] ( Admin Group ) 
⎆ ${prefix}nobadword [1/0] ( Admin Group ) 
⎆ ${prefix}simih [1/0] ( Admin Group ) 
⎆ ${prefix}linknotify [1/0] ( Admin Group )
⎆ ${prefix}antifasttour [1/0] ( Admin Group )
⎆ ${prefix}promote [@tag] ( Admin Group ) 
...
⎆ ${prefix}nyewabot [ Waktu sewa ] ( Coming Soon!! ) 
⎆ ${prefix}buypremium [ Waktu Premium ] ( Coming Soon!! ) 
Note : Cek Harga GS ${prefix}garenashell
Top up saldo? wa.me/62895325677273
Via : Telkomsel Transfer ( Rate 75% ) 
Bank ( Tanpa Pajak ) 
Qris ( Pajak 1.5% ) 
...
case 'buy33gs':
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
				const payoutxx = body.slice(8) 
				const saldoPerlimitt = 18560
				const tlotalx = saldoPerlimitt * payoutxx
