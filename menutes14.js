...
pushnem = client.contacts[num] != undefined ? client.contacts[num].notify = undefined ? PhoneNumber('+' + num.replace('@s.whatsapp.net', '')).getNumber('international') : client.contacts[num].notify || client.contacts[num].vname : PhoneNumber('+' + num.replace('@s.whatsapp.net', '')).getNumber('international')
			try {
				...
				.on('error', () => reply('error'))
				.on('exit', () => {
			client.sendMessage(mdata.id, fs.readFileSync('wel.jpg'), MessageType.image, {caption: `*Dadah Beban group 👋*@${num.split('@')[0]}\n`, contextInfo: { mentionedJid: [num] }})
			})
		}
	} catch (e) {
		console.log(e)
...
/*case 'shutdown':
				...
				if (!arg) return
				client.sendMessage(from, `BOT STATUS : OFF`, text, {quoted: freply})
				exec(kontol.shutdiwnkska)
			*/
				break
/////////////////////////////// SELF TESTER ///////////////////
case 'sewacheck':
...
case 'caklontong':
                    if (!isRegistered) return reply(ind.noregis())
	            	if (isLimit(sender)) return reply(ind.limitend(pushname))
					anu = await fetchJson(`https://api.lolhuman.xyz/api/tebak/caklontong?apikey=${LolhumanKey}`)
			...
			/*case 'dare':
				...
				case 'family100':
					...
					anu = await fetchJson(`https://api.lolhuman.xyz/api/tebak/family100?apikey=${LolhumanKey}`)
...
case 'buy33gs':
				...
				const tlotal = saldoPerlimitk * payoutku
